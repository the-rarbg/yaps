...
import {
    BiSolidSkipNextCircle, BiSolidSkipPreviousCircle,
} from 'react-icons/bi'
...
const Tv = () => {
    ...
    useEffect(() => {
        let temp = []
        if (data) if (data.detail.seasons[0].name.includes('Specials')) for (let i = 1; i < data.detail.seasons.length; i++) {
            temp.push(i)
        } else for (let i = 1; i <= data.detail.seasons.length; i++) {
            temp.push(i)
        }
        SetSeason(temp)
        let episode = []
        ...
        if (data) {
            if (data.detail.seasons[0].name.includes('Specials')) {
                for (let k = 0; k < temp.length; k++) {
                    for (let i = 1; i < data.detail.seasons[c].episode_count + 1; i++) episode.push(i)
                    episodes[c] = episode
                    c++
                    episode = []
                }
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem('userWatched')) {
            ...
            if (id || tmdb) localStorage.setItem('userWatched', JSON.stringify({
    ...
    useEffect(() => {
        if (data !== undefined) {
            ...
            if (videoServer.includes('moviesapi.club')) setVideoServer(`https://moviesapi.club/tv/${tmdb}-${tvDetails.season}-${tvDetails.episode}`)
    ...
    return (<>
        ...
        <div
            className={`left-0  top-0 z-[997] w-full bg-black transition duration-300 ease-in-out ${lightStatus ? 'opacity-1 fixed h-screen ' : 'h-0 w-0 opacity-0'}`}></div>
        <div
            ...
            <div
                className={'flex w-full flex-row items-center justify-between gap-10 bg-transparent p-4 pl-0'}>
                <div>
                    <div
                        ...
                        <div className={'relative'}>
                            ...
                            />
                            <div
                                className={'tooltip-div absolute left-5 z-50 hidden w-10 bg-[#222222] text-center'}>
                                Prev
                            </div>
                        </div>
                        <div
                                ...
                                if (episodes[tvDetails.season]) {
                                    if (episodes[tvDetails.season].indexOf(current_Episode) === episodes[tvDetails.season].length - 1) {
                                        if (season.indexOf(tvDetails.season) !== season.length - 1) {
                                            setTvDetails({
                                                season: current_Season + 1, episode: 1,
                                            })
                                        }
                                    } else {
                                        setTvDetails(prev => {
                            ...
                            <BiSolidSkipNextCircle
                                size={35}
                                className={`${nextBtn ? 'block' : 'hidden'} control-button transition duration-300 ease-in-out hover:scale-125`}
                            />
                            <div
                                className={'tooltip-div absolute left-5 z-50 hidden w-10 bg-[#222222] text-center'}>
                                Next
                ...
                <div
                    ...
                    <div
                        ...
                        className={'flex flex-row items-center gap-1 transition duration-300 ease-in-out hover:cursor-pointer hover:text-orange-500'}>
                        <BsFillLightbulbFill/>
                        <span>Light</span>
                    </div>
                    <div className={'relative w-32 '}>
                        <div
                            className={'z-50 flex cursor-pointer flex-row items-center justify-center   gap-2 rounded bg-[#151515] text-white  outline-0 dark:bg-inherit'}
                            onClick={() => {
                  ...
                  {seasonDropDown ? <AiOutlineDown/> : <AiOutlineRight/>}
                        ...
                        <div
                            className={` absolute  z-40  h-44 w-44  flex-col  items-center justify-center overflow-y-scroll font-normal text-white transition duration-300 ease-in-out ${seasonDropDown ? 'opacity-1 translate-y-0' : 'translate-y-[-150%]   opacity-0'} `}>
                            <ul className={''}>
                                {season !== [] ? (season.map((season, index) => {
                                    return (<li
                                        ...
                                        className={`dropdown-scroll relative flex  h-12 w-full items-center justify-evenly bg-app-semi-dark-blue pl-2 hover:cursor-pointer hover:bg-amber-700 ${tvDetails.season === season ? 'bg-amber-700' : ''}`}>
                    ...
                    <div className={'relative w-32 '}>
                        <div
                            className={'z-50 flex flex-row items-center justify-center gap-2 rounded   bg-[#151515] text-white dark:bg-inherit'}
                            onClick={() => {
                  ...
                  {episodeDropDown ? <AiOutlineDown/> : <AiOutlineRight/>}
                        ...
                        <div
                            className={` absolute   z-40 h-44 w-44 flex-col  items-center  justify-center overflow-y-scroll bg-app-semi-dark-blue font-normal text-white transition duration-300 ease-in-out ${episodeDropDown ? 'opacity-1 translate-y-0' : 'translate-y-[-150%]   opacity-0'} `}>
                            <ul className={''}>
                                {episodes[tvDetails.season] ? (episodes[tvDetails.season].map((episode, index) => {
                                    return (<li
                                        ...
                                        className={`dropdown-scroll relative  flex h-12 w-full items-center justify-evenly pl-2 hover:cursor-pointer hover:bg-amber-700 ${tvDetails.episode === episode ? 'bg-amber-700' : ''}`}>
            ...
            <div
                ...
                <div className={'flex h-full w-full flex-row rounded '}>
                    ...
                    <div className={` h-full w-2/3`}>
                        <div
                            className={'flex h-10 w-full flex-row flex-wrap items-center justify-start gap-7 p-5'}>
                            {servers_.map((server, index) => {
                                return (<div
                                    onClick={() => {
                                        if (server.servername === 'Vidsrc.me') setVideoServer(`${server.link}${tmdb ? `tmdb=${tmdb}` : `imdb=${id}`}&season=${tvDetails.season}&episode=${tvDetails.episode}`)
                                        else if (server.servername === 'Vidsrc.to') setVideoServer(`${server.link}${tmdb ? `${tmdb}` : `${id}`}/${tvDetails.season}/${tvDetails.episode}`)
                                        else if (server.servername === 'Moviesapi.club') setVideoServer(`${server.link}${tmdb}-${tvDetails.season}-${tvDetails.episode}`)
                                        else setVideoServer(`${server.link}${tmdb}&season=${tvDetails.season}&episode=${tvDetails.episode}`)
                                    }}
                                    className={`  ${videoServer.includes(server.servername.toLowerCase()) ? 'bg-app-greyish-blue' : ''} h-8 w-max rounded p-3 text-center transition duration-300 ease-in-out hover:scale-110 hover:cursor-pointer dark:text-white `}
                                    key={index}>
                                    {server.servername}
                                </div>)
                            })}
